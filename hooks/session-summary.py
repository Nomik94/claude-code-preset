...
On every Stop event, parses the current session transcript and writes
...
def generate_summary(path, user_messages, files_edited, files_created):
    ...
    display_msgs, _ = truncate_messages(user_messages)
    ...
    all_files = [(f, "edit") for f in sorted(files_edited)] + [
