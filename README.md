...
clone → `install.sh` 실행만으로 26개 skill, 7개 agent, 6개 hook이 설정됩니다.
...
- **Phase Gate**: 기획 → 스펙 → 설계 → 구현 → 검증 → 배포 풀사이클 (Spec-Driven Development)
...
│
├── skills/ (26개 — on-demand, 키워드 매칭 시 로드)
│   │
│   │  # 공통 워크플로우
│   ├── spec/                          # Spec-Driven Development (constitution + 기능 명세)
│   ├── confidence-check/              # 구현 전 신뢰도 평가 (≥90%)
│   ├── verify/                        # 완료 후 7단계 검증
│   ├── checkpoint/                    # 위험 작업 전 롤백 포인트
│   ├── audit/                         # 커밋/PR 전 규칙 검증
│   ├── build-fix/                     # 빌드 에러 자동 수정
│   ├── feature-planner/               # Phase 기반 기능 계획 (스펙 연동)
│   ├── gap-analysis/                  # 스펙 vs 구현 Match Rate
...
```
기획(planner) → 스펙(spec) → 설계(architect) → 구현(engineer) → 검증(reviewer/debugger) → 배포(devops)
```
...
| Agent | Phase | 키워드 |
|-------|-------|--------|
| **planner** | 기획 | 기획, PRD, 요구사항, 비즈니스, 사업성, MVP, 스코프, 뭘 만들지, 우선순위 |
| **architect** | 설계 | 설계, 아키텍처, 스키마, ERD, 구조, 데이터 모델, API 설계, 테이블 설계, 어떻게 나눌지 |
| **engineer** | 구현 | 구현, 만들어, 추가해, implement, create, 개발, 코딩, 작성해, 연동, 넣어줘 |
| **reviewer** | 검증 | 리뷰, 코드 품질, 리팩토링, 검토, 기술 부채, 봐줘, 괜찮은지, 문제 없는지 |
| **debugger** | 검증 | 버그, 디버깅, 왜 안 돼, 에러, 안 됨, 실패, 오류, 깨짐, 터짐, 느려짐, 동작 안 해 |
| **devops** | 배포 | Docker, 배포, CI/CD, 인프라, 컨테이너, 모니터링, 로깅, 알림, nginx, 운영 |
| **writer** | 공통 | 문서, README, API 문서, ADR, 문서화, 가이드, 런북, 정리해줘 |
...
| **planner** | spec, feature-planner, gap-analysis | — |
...
| Skills | 26개 (공통 10 + BE 5 + FE 4 + 인프라 3 + 스캐폴딩 2 + on-demand 2) |
| Gotchas | 26개 (전 스킬 Claude 빈출 실수 패턴) |
...
| [GitHub — Spec Kit](https://github.com/github/spec-kit) | spec 스킬의 Spec-Driven Development 방법론 (Constitution, Specification-First) |
