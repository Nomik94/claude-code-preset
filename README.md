...
**핵심 아이디어**: CLAUDE.md (~197줄)만 항상 로드되고, 나머지는 **Skill 시스템**으로 필요할 때만 로드됩니다.
...
```
router → application/service → domain/entity → infrastructure/repository
```
...
```
~/.claude/
├── CLAUDE.md                  # Core config (항상 로드, ~201줄)
│
...
기존 글로벌 설정 대비 **상시 로드 ~90% 감소** (~2,400줄 → ~197줄).
...
| 기능 완료 후 | `/verify` | 6단계 검증 (테스트, 린트, 타입, 보안 등) |
...
복잡한 작업은 전문 에이전트에게 위임합니다. `/명령`으로 직접 호출도 가능합니다:

```
사용자: "주문 시스템 설계하고 구현해줘"
  → /engineer 로드 → Task tool로 engineer 에이전트 스폰
  → 설계(API+DB) → TDD 구현 → 보안/성능 검증

사용자: "/code-review"
  → code-reviewer 에이전트 스폰 → 5-카테고리 스코어링
```

| 호출 | Agent | 역할 |
|------|-------|------|
| `/engineer` | engineer | 설계 + 구현 (API, DB, 아키텍처, 성능, 보안, 테스트) |
| `/code-review` | code-reviewer | 코드/PR 리뷰, 스코어링 |
| `/root-cause` | root-cause-analyst | 버그 원인 추적, 가설 검증 |
| `/devops` | devops-architect | Docker, CI/CD, 인프라, 배포 |
| `/docs` | technical-writer | README, ADR, API 문서, 변경 로그 |
